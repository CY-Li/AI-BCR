...
## v1.0.14 — 2026-09-14 — 掃描設定、名片備註與操作流程改善

### 掃描與設定

- 新增 Scan Settings，提供 200／300／600 dpi、彩色／灰階／黑白，以及自動裁切、自動校正與自動旋轉選項。
- 掃描設定沿用 `appsettings.json` 保存，讀取失敗或內容無效時安全回退至預設值；保存失敗時保留上一份有效設定。
- Settings 工作區重新整理導覽與卡片層級，補充 Scan、General、Duplicate Detection 與 About 的英／日文介面文字。
- About 頁新增應用程式版本與已連接掃描器韌體資訊。

### 名片備註與重複資料

- 名片新增備註時間軸，支援建立、編輯與刪除，並依最後修改時間排序。
- 刪除備註加入不可復原確認提示，避免操作員誤刪。
- Replace 重複名片時會合併所有被取代名片的備註，以備註 ID 去重並保留最新排序。
- 擴充重複比對與備註合併測試，涵蓋多筆取代、備註去重及時間順序。

### 操作介面與導覽

- 頂部、All Cards 右側面板與 Card Detail 頁統一使用掃描框＋星芒 AI 圖示，並依按鈕尺寸校正視覺大小。
- 改善主畫面、清單與側邊預覽的名片縮圖解碼尺寸，降低高解析度原圖大倍率縮放造成的細字取樣問題。
- 修正 Today、Within 3 days、Within 7 days 日期篩選後，側欄選取狀態錯誤跳回 All cards 的問題。
- 修正 Tag 篩選後未保留所選 Tag 高亮的問題，並避免重複觸發篩選。
- 暫時隱藏尚未實作的 By Company、By Name 與 Help，避免顯示不可用入口；既有程式碼保留供後續啟用。

### 相容性與驗證

- 未修改 OCR JSON schema、掃描器協定、名片持久化欄位名稱或外部 API。
- 版本同步更新為 `1.0.14`；MSIX 套件版本為 `1.0.14.0`。
